...
n_x = 10
n_y = 10
n_clust = 12 # For distinct colours visualisation purpose, we will not compute more than 12 superclusters
...
supercl <- data.frame(set = as.character(clusters))

library(dplyr)
som.cl.colours <- inner_join(supercl, colours)

plot(som_model, type="codes", shape="straight", palette.name = rainbow, main = "SOM components", bgcol=som.cl.colours$colour)
add.cluster.boundaries(som_model, clusters, lwd=2, col='black')

legend(x=-1.5, y = 8, legend = colours$set, fill = colours$colour, title = 'cluster')
