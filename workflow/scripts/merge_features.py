...
for input_file in input_files_orthogroups:
    df = pd.read_csv(open(input_file), sep='\t') # I don't use the input_file variable name here because if given a string, pandas.read_csv() will automatically close the file, which is instead needed to be open for later
